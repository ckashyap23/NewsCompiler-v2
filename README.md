...
- Runs daily topic research with OpenAI web search.
...
Initialize or verify the DB table:
...
For Render/headless cron, paste a valid token into `LINKEDIN_ACCESS_TOKEN` and set `LINKEDIN_AUTHOR_URN`.
...
topic_research.py            OpenAI web-search research call
newsletter_compiler.py       OpenAI structured newsletter synthesis
database.py / models.py      SQLAlchemy storage layer
send_email.py                Gmail delivery
linkedin_auth.py             Local LinkedIn OAuth helper
